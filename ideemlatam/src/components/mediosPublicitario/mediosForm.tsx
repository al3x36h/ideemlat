...
type Props = {
  defaultData: MedioPublicitario | null;
  onClose: () => void;
  onSave: (data: MedioPublicitario) => Promise<void> | void;
};

export default function MediosForm({
  defaultData,
  onClose,
  onSave,
}: Props) {
  const [form, setForm] = useState<MedioPublicitario>(
    defaultData || { codigo: 0, nombre: "", activo: false }
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end gap-2">
        ...
        >
          {defaultData ? "Guardar Cambios" : "Crear Area de Conocimiento"}
        </button>
      </div>
    </form>
  );
